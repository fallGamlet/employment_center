  ...
  Collapse.prototype = {
    ...
    show: function () {
      ...
      if (!this.transitioning) {
        if (dimension = this.dimension(), scroll = $.camelCase(['scroll',
        dimension].join('-')), actives = this.$parent && this.$parent.find('> .accordion-group > .in'), actives && actives.length) {
  ...
  ScrollSpy.prototype = {
    ...
    refresh: function () {
      ...
      $targets = this.$body.find(this.selector) .map(function () {
        ...
        return $href && $href.length && [[$href.position() .top + self.$scrollElement.scrollTop(),
        href]] || null
      }) .sort(function (a, b) {
        return a[0] - b[0]
      }) .each(function () {
...
!function ($) {
  ...
  Typeahead.prototype = {
    ...
    sorter: function (items) {
      for (var item, beginswith = [
      ], caseSensitive = [
      ], caseInsensitive = [
      ]; item = items.shift(); ) item.toLowerCase() .indexOf(this.query.toLowerCase()) ? ~item.indexOf(this.query) ? caseSensitive.push(item)  : caseInsensitive.push(item)  : beginswith.push(item);
